...
<head>
  ...
  <style>
    *, *::before, *::after { margin:0; padding:0; box-sizing:border-box; }
    ...
    .join-btn { display:inline-flex; align-items:center; gap:.6rem; padding:.75rem 1.6rem; border-radius:12px; font-family:'Syne',sans-serif; font-weight:700; font-size:.9rem; text-decoration:none; border:none; cursor:pointer; transition:transform .2s,box-shadow .2s,opacity .2s; }
    .join-btn:hover { transform:translateY(-2px); }
    .join-btn:disabled { opacity:.5; cursor:not-allowed; transform:none; }
    .btn-100ms { background:linear-gradient(135deg,#ef4444,#dc2626); color:#fff; box-shadow:0 4px 20px rgba(239,68,68,.3); }
    ...
    /* 100ms PLAYER PANEL */
    .hms-player-wrap { margin-top:1.4rem; border-radius:16px; overflow:hidden; border:1px solid rgba(239,68,68,.2); background:#000; position:relative; }
    .hms-player-header { display:flex; align-items:center; justify-content:space-between; padding:.7rem 1.2rem; background:rgba(239,68,68,.08); border-bottom:1px solid rgba(239,68,68,.12); }
    .hms-player-title { display:flex; align-items:center; gap:.5rem; font-family:'Syne',sans-serif; font-weight:700; font-size:.78rem; color:#f87171; }
    .hms-live-dot { width:7px; height:7px; border-radius:50%; background:#ef4444; animation:blink 1.2s infinite; flex-shrink:0; }
    .hms-leave-btn { background:rgba(239,68,68,.15); border:1px solid rgba(239,68,68,.3); color:#f87171; font-size:.72rem; font-family:'Syne',sans-serif; font-weight:700; padding:.3rem .75rem; border-radius:8px; cursor:pointer; transition:background .2s; }
    .hms-leave-btn:hover { background:rgba(239,68,68,.3); }
    .hms-iframe-container { width:100%; height:540px; }
    .hms-iframe-container iframe { width:100%; height:100%; border:none; display:block; }
    ...
    @media(max-width:900px) {
      ...
      .hms-iframe-container { height:360px; }
    }
  </style>
</head>
<body>
...
<div class="page-wrap">
  <div class="hero">
    <div class="hero-inner">
      <div class="hero-tag">
        ...
        Sesiones en Vivo · 100ms
      </div>
      <h1 class="hero-title">Charlas <span>Constructivas</span></h1>
      <p class="hero-desc">Sesiones interactivas en vivo. Cámara, micrófono y chat directamente en tu navegador — sin instalaciones.</p>
    </div>
  </div>
  ...
  <div class="main-grid">
    <div class="left-col">
      ...
      <!-- Panel 100ms (oculto hasta unirse) -->
      <div id="hmsPlayerWrap" style="display:none">
        <div class="hms-player-wrap">
          <div class="hms-player-header">
            <div class="hms-player-title">
              <div class="hms-live-dot"></div>
              Sala activa · 100ms
            </div>
            <button class="hms-leave-btn" onclick="salirSala100ms()">✕ Salir de la sala</button>
          </div>
          <div class="hms-iframe-container" id="hmsIframeContainer">
      ...
      <div id="agendaBox" style="display:none">
        <div class="sec-label" style="margin-top:1.8rem">
...
const PN = { zoom:'Zoom', meet:'Google Meet', teams:'Microsoft Teams', webex:'Webex', otro:'En línea', '100ms':'100ms (integrado)' };
const PB = { zoom:'btn-zoom', meet:'btn-meet', teams:'btn-teams', webex:'btn-webex', otro:'btn-otro', '100ms':'btn-100ms' };

let currentCharla = null;
...
function render(c) {
  ...
  // ─── LÓGICA DE ACCESO ────────────────────────
  let joinHtml = '';
  const plat = c.plataforma || '100ms';

  if (plat === '100ms' || (!c.link_reunion && plat !== 'zoom' && plat !== 'meet' && plat !== 'teams' && plat !== 'webex')) {
    // Sala 100ms integrada
    if (c.estado === 'en_vivo') {
      joinHtml = `
        <div class="join-area">
          <button class="join-btn btn-100ms" id="joinBtn100ms" onclick="unirse100ms('${c.titulo}', ${c.id})">
            📡 Unirme a la sesión
          </button>
        </div>
        <div style="font-size:.75rem;color:var(--text-muted);margin-top:.6rem">
          Sin instalaciones · Cámara y micrófono en el navegador
        </div>`;
    } else if (c.estado === 'programada') {
      joinHtml = `<div class="link-lock">🔒 La sala abrirá cuando el instructor inicie la sesión</div>`;
    } else {
      joinHtml = `<div class="link-lock" style="color:var(--text-muted);border-color:var(--border-soft);background:transparent">Sesión finalizada. Ver grabación abajo.</div>`;
    }
  } else if (c.link_reunion) {
    // Link externo
    joinHtml = `
      <div class="join-area">
        <a href="${c.link_reunion}" target="_blank" rel="noopener" class="join-btn ${PB[plat]||'btn-otro'}">${PI[plat]||'🌐'} Unirme a la sesión</a>
    ...
    joinHtml = `<div class="link-lock">🔒 El enlace estará disponible 24h antes de la sesión</div>`;
  }
  ...
  box.innerHTML = `
    <div class="charla-card">
      ...
      <div class="card-meta">
        ...
        <div class="meta-item"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="2" y="3" width="20" height="14" rx="2"/></svg><strong>${PN[plat]||'100ms integrado'}</strong></div>
      </div>
      <div class="card-body">
        ${c.descripcion ? `<p class="card-desc">${c.descripcion}</p>` : ''}
        ${joinHtml}
        ${c.link_grabacion ? `<div style="margin-top:1rem"><a href="${c.link_grabacion}" target="_blank" rel="noopener" class="copy-btn">▶ Ver grabación de esta sesión</a></div>` : ''}
      </div>
    </div>`;
  ...
  document.getElementById('qi-plat').textContent = PN[plat] || '100ms integrado';
...
// ──────────────────────────────────────────────
//  100ms: UNIRSE COMO LEARNER
// ──────────────────────────────────────────────
async function unirse100ms(titulo, charlaId) {
  const btn = document.getElementById('joinBtn100ms');
  ...
  try {
    ...
    try {
      ...
      if (storedUser) {
        ...
        userName = u.nombre ? `${u.nombre} ${u.apellido||''}`.trim() : 'Estudiante';
      }
    } catch(_) {}

    const res = await fetch(`${API_100MS}?topic=${encodeURIComponent(topic)}&role=learner`, {
      credentials: 'include',
      headers: token ? { 'Authorization': 'Bearer ' + token } : {}
    });
    const json = await res.json();

    if (!json.ok) throw new Error(json.error || 'No se pudo obtener acceso a la sala');

    let iframeUrl = '';
    if (json.data.prebuiltUrl) {
      iframeUrl = json.data.prebuiltUrl
      ...
      throw new Error('El servidor no devolvió URL ni Room Code válidos.');
    }

    // Inyectar iframe
    document.getElementById('hmsIframeContainer').innerHTML = `
      <iframe
        title="100ms-learner"
        src="${iframeUrl}"
      ...
      ></iframe>`;
    document.getElementById('hmsPlayerWrap').style.display = 'block';
    document.getElementById('liveBadge').classList.add('show');

    // Ocultar el botón de unirse (ya está adentro del player)
    const charlaBox = document.getElementById('charlaBox');
    if (charlaBox) charlaBox.style.opacity = '0.6';

    showToast('✅ Conectado. ¡Bienvenido a la sesión!');
...
// ──────────────────────────────────────────────
//  100ms: SALIR DE LA SALA
// ──────────────────────────────────────────────
function salirSala100ms() {
  document.getElementById('hmsPlayerWrap').style.display = 'none';
  document.getElementById('hmsIframeContainer').innerHTML = '';
  document.getElementById('liveBadge').classList.remove('show');
  const charlaBox = document.getElementById('charlaBox');
  if (charlaBox) charlaBox.style.opacity = '1';
  const btn = document.getElementById('joinBtn100ms');
  if (btn) { btn.disabled = false; btn.textContent = '📡 Unirme a la sesión'; }
