...
<aside class="sidebar" id="sidebar">
  ...
  <nav class="sidebar-nav">
    ...
    <a class="nav-item" data-section="charlas">
      ...
      Charlas en Vivo
      <span class="nav-count" id="cnt-charla" style="background:rgba(239,68,68,.2);color:#f07a7a">📡</span>
    </a>
    <a class="nav-item" data-section="live">
      <span class="nav-icon"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22.54 6.42a2.78 2.78 0 00-1.95-1.96C18.88 4 12 4 12 4s-6.88 0-8.59.46A2.78 2.78 0 001.46 6.42 29 29 0 001 12a29 29 0 00.46 5.58 2.78 2.78 0 001.95 1.96C5.12 20 12 20 12 20s6.88 0 8.59-.46a2.78 2.78 0 001.95-1.96A29 29 0 0023 12a29 29 0 00-.46-5.58z"/><polygon points="9.75 15.02 15.5 12 9.75 8.98 9.75 15.02"/></svg></span>
      Live YouTube
    </a>
    <a class="nav-item" data-section="calendario">
      ...
      Calendario
      <span class="nav-count" id="cnt-eventos">—</span>
    </a>
  ...
  <div class="content">
    ...
    <div class="section" id="sec-charlas">
      <div class="page-header">
        ...
        <h1 class="page-title">Charlas <em>por Curso</em></h1>
        <p class="page-sub">Selecciona un curso para agendar o iniciar su charla en vivo.</p>
      </div>
      <div class="grid-3" id="charlas-cursos-grid">
    ...
    <div class="section" id="sec-live">
      <div class="page-header">
        <div class="page-label">Transmisión</div>
        <h1 class="page-title">Live <em>YouTube</em></h1>
        <p class="page-sub">Configura y gestiona la transmisión en vivo pública.</p>
      </div>
      ...
      <div style="display:grid;grid-template-columns:1fr 320px;gap:1.4rem;align-items:start">
        ...
        <div class="card">
          <div class="card-header">
            <div class="card-title">Configuración del Live</div>
            <a href="/live" target="_blank" rel="noopener" style="font-size:.75rem;color:var(--teal);text-decoration:none;display:flex;align-items:center;gap:.3rem">
          ...
          <div class="form-group" style="margin-bottom:1rem">
            <label class="form-label">ID o URL del video de YouTube</label>
            <input type="text" class="form-input" id="live-yt-id" placeholder="Ej: dQw4w9WgXcQ  o  https://youtube.com/watch?v=...">
            <div style="font-size:.72rem;color:var(--muted);margin-top:.3rem">Puedes pegar el ID (11 caracteres) o la URL completa.</div>
          </div>

          <div class="form-group" style="margin-bottom:1rem">
            <label class="form-label">Título del live</label>
            <input type="text" class="form-input" id="live-titulo" placeholder="Ej: Workshop 1 — BIM Revit en vivo">
          </div>

          <div class="form-group" style="margin-bottom:1.2rem">
            <label class="form-label">Descripción (opcional)</label>
            <textarea class="form-textarea" id="live-desc" rows="3" placeholder="Describe brevemente el contenido del live..."></textarea>
          </div>

          <div style="display:flex;gap:2rem;margin-bottom:1.4rem;flex-wrap:wrap">
            <label style="display:flex;align-items:center;gap:.6rem;cursor:pointer;font-size:.84rem">
              <div style="position:relative">
                <input type="checkbox" id="live-activo" style="position:absolute;opacity:0;width:100%;height:100%;cursor:pointer;margin:0;z-index:1">
                <div id="live-activo-track" style="width:36px;height:20px;background:var(--border);border-radius:10px;transition:background .2s;position:relative">
                  <div id="live-activo-thumb" style="position:absolute;top:3px;left:3px;width:14px;height:14px;background:#fff;border-radius:50%;transition:transform .2s;pointer-events:none"></div>
                </div>
              </div>
              <span>Live activo (visible al público)</span>
            </label>
            <label style="display:flex;align-items:center;gap:.6rem;cursor:pointer;font-size:.84rem">
              <div style="position:relative">
                <input type="checkbox" id="live-chat" checked style="position:absolute;opacity:0;width:100%;height:100%;cursor:pointer;margin:0;z-index:1">
                <div id="live-chat-track" style="width:36px;height:20px;background:var(--teal);border-radius:10px;transition:background .2s;position:relative">
                  <div id="live-chat-thumb" style="position:absolute;top:3px;left:19px;width:14px;height:14px;background:#fff;border-radius:50%;transition:transform .2s;pointer-events:none"></div>
                </div>
              </div>
              <span>Mostrar chat de YouTube</span>
            </label>
          </div>

          <div style="display:flex;gap:.6rem;align-items:center">
            <button id="live-save-btn" onclick="saveLiveConfig()" class="btn btn-primary">
              <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="20 6 9 17 4 12"/></svg>
              Guardar configuración
            </button>
            <span id="live-save-msg" style="font-size:.78rem;color:var(--teal);display:none">✓ Guardado</span>
          </div>
        </div>

        <div>
          <div class="card" style="margin-bottom:1.2rem">
            <div class="card-header"><div class="card-title">Estado actual</div></div>
            <div style="text-align:center;padding:.5rem 0">
              <div style="font-size:2.5rem;margin-bottom:.6rem" id="live-estado-icon">⏳</div>
              <div style="font-family:'Syne',sans-serif;font-weight:700;font-size:.9rem" id="live-estado-texto">Cargando...</div>
              <div style="font-size:.75rem;color:var(--muted);margin-top:.3rem" id="live-estado-id"></div>
            </div>
          </div>

          <div class="card">
            <div class="card-header"><div class="card-title">Cómo funciona</div></div>
            <div style="font-size:.8rem;color:var(--muted);line-height:1.8">
              <p>1. Inicia tu transmisión en YouTube Studio.</p>
              <p>2. Copia el ID o URL del video.</p>
              <p>3. Pégalo y activa <strong style="color:var(--text)">Live activo</strong>.</p>
              <p>4. Los visitantes lo verán en <code style="background:rgba(93,230,212,.1);padding:.1rem .4rem;border-radius:4px;font-size:.75rem">/live</code>.</p>
            </div>
          </div>
        </div>
    ...
    <div class="section" id="sec-calendario">
      <div class="page-header">
        <div class="page-label">Eventos</div>
        <h1 class="page-title">Calendario de <em>Eventos</em></h1>
        <p class="page-sub">Gestiona workshops, charlas, lives y eventos especiales.</p>
      </div>

      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1.3rem;flex-wrap:wrap;gap:.8rem">
        <div style="display:flex;gap:.5rem;flex-wrap:wrap">
          <select class="form-select" id="ev-filter-tipo" style="width:160px" onchange="loadEventos()">
            <option value="">Todos los tipos</option>
            <option value="live">Live YouTube</option>
            <option value="workshop">Workshop</option>
            <option value="charla">Charla en vivo</option>
            <option value="especial">Evento especial</option>
          </select>
          <a href="/calendario" target="_blank" rel="noopener" class="btn btn-ghost btn-sm" style="display:flex;align-items:center;gap:.35rem">
        ...
        <button class="btn btn-primary" onclick="abrirModalEvento()">+ Nuevo evento</button>
      </div>
      ...
      <div class="card">
        <table class="data-table">
          <thead>
            <tr>
              <th>Evento</th><th>Tipo</th><th>Fecha</th><th>Visibilidad</th><th>Curso</th><th></th>
            </tr>
          </thead>
  ...
  <div class="modal" style="max-width:560px">
    ...
    <div class="form-row">
      <div class="form-group"><label class="form-label">Título *</label>
        <input class="form-input" id="ev-titulo" placeholder="Ej: Workshop 1 — BIM Revit en vivo">
      </div>
    </div>
    <div class="form-row cols-2">
      <div class="form-group"><label class="form-label">Tipo *</label>
      ...
      <div class="form-group"><label class="form-label">Curso (opcional)</label>
        <select class="form-select" id="ev-curso">
          <option value="">Público (sin curso)</option>
        </select>
      </div>
    </div>
    <div class="form-row cols-2">
      <div class="form-group"><label class="form-label">Fecha y hora inicio *</label>
        <input class="form-input" id="ev-inicio" type="datetime-local">
      </div>
      <div class="form-group"><label class="form-label">Fecha y hora fin</label>
        <input class="form-input" id="ev-fin" type="datetime-local">
      </div>
    </div>
    <div class="form-row">
      <div class="form-group"><label class="form-label">Link del evento (meet, zoom, youtube...)</label>
        <input class="form-input" id="ev-link" placeholder="https://...">
      </div>
    </div>
    <div class="form-row">
      <div class="form-group"><label class="form-label">Descripción</label>
        <textarea class="form-textarea" id="ev-desc" placeholder="Descripción breve del evento..."></textarea>
      </div>
    </div>
    <div class="form-row cols-2">
      <div class="form-group">
        <label style="display:flex;align-items:center;gap:.5rem;cursor:pointer;font-size:.84rem;margin-top:.4rem">
          <input type="checkbox" id="ev-publico" checked style="accent-color:var(--teal);width:15px;height:15px">
          <span>Visible para todos (público)</span>
        </label>
        <div style="font-size:.7rem;color:var(--muted);margin-top:.3rem">Desmarca para que solo lo vean los inscritos al curso.</div>
      </div>
      <div class="form-group"><label class="form-label">Color (opcional)</label>
        <input class="form-input" id="ev-color" type="color" value="#0aab96" style="height:38px;padding:.3rem">
      </div>
    </div>
...
// ══ NAVEGACIÓN ══
const LABELS = {dashboard:'Dashboard', usuarios:'Usuarios', cursos:'Cursos', tareas:'Tareas', charlas:'Charlas en Vivo', live:'Live YouTube', calendario:'Calendario'};
...
navigate = function(id) {
  ...
  if (id === 'live')       loadLiveConfig();
  if (id === 'calendario') loadEventos();
};

// ══ LIVE YOUTUBE ══
function syncToggle(checkId, trackId, thumbId) {
  const chk = document.getElementById(checkId);
  const track = document.getElementById(trackId);
  const thumb = document.getElementById(thumbId);
  if (!chk || !track || !thumb) return;
  function update() {
    track.style.background = chk.checked ? 'var(--teal)' : 'rgba(13,110,100,.14)';
...
async function loadLiveConfig() {
  try {
    ...
    if (!j.ok) return;
    const cfg = j.data;
    document.getElementById('live-yt-id').value = cfg.youtube_id  || '';
    document.getElementById('live-titulo').value = cfg.titulo     || '';
    document.getElementById('live-desc').value   = cfg.descripcion || '';
    const ac = document.getElementById('live-activo');
    const ch = document.getElementById('live-chat');
    if (ac) { ac.checked = !!cfg.activo;      ac.dispatchEvent(new Event('change')); }
    if (ch) { ch.checked = !!cfg.mostrar_chat; ch.dispatchEvent(new Event('change')); }
    updateLiveEstado(cfg);
  } catch(e) {}
}

function updateLiveEstado(cfg) {
  const icon  = document.getElementById('live-estado-icon');
  const texto = document.getElementById('live-estado-texto');
  const idEl  = document.getElementById('live-estado-id');
  if (!icon || !texto) return;
  if (cfg.activo && cfg.youtube_id) {
    icon.textContent  = '🔴';
    texto.textContent = 'Live activo y visible';
    texto.style.color = '#f07a7a';
    idEl.textContent  = 'ID: ' + cfg.youtube_id;
  } else if (cfg.youtube_id) {
    icon.textContent  = '⏸️';
    texto.textContent = 'Video configurado (inactivo)';
    texto.style.color = '';
    idEl.textContent  = 'ID: ' + cfg.youtube_id;
  } else {
    icon.textContent  = '📡';
    texto.textContent = 'Sin video configurado';
    texto.style.color = '';
    idEl.textContent  = '';
  }
}
...
async function saveLiveConfig() {
  ...
  btn.disabled = true;
  const body = {
    youtube_id:   document.getElementById('live-yt-id').value.trim(),
    titulo:       document.getElementById('live-titulo').value.trim(),
    descripcion:  document.getElementById('live-desc').value.trim(),
    activo:       document.getElementById('live-activo').checked,
    mostrar_chat: document.getElementById('live-chat').checked,
  };
  try {
    ...
    if (j.ok) {
      ...
      updateLiveEstado(body);
      toast('✅ Configuración del live guardada');
    } else {
      toast(j.error || 'Error al guardar', true);
    }
...
// ══ CALENDARIO / EVENTOS ══
const TIPO_LABELS_ADMIN = { live:'🔴 Live', workshop:'🏗️ Workshop', charla:'📡 Charla', especial:'⭐ Especial' };
let editEventoId = null;

async function loadEventos() {
  const tipo = document.getElementById('ev-filter-tipo')?.value || '';
  const url  = `/Php/eventos.php${tipo ? '?tipo='+tipo : ''}`;
  try {
    const token = localStorage.getItem('cv_token') || '';
    ...
    const evs = j.data.eventos;
    const cnt = document.getElementById('cnt-eventos');
    ...
    if (!evs.length) {
      tbody.innerHTML = `<tr><td colspan="6" style="text-align:center;color:var(--muted);padding:2rem">No hay eventos. Crea el primero.</td></tr>`;
      return;
    }

    tbody.innerHTML = evs.map(ev => {
      const fi = new Date(ev.fecha_inicio);
      const fecha = `${fi.getDate()} ${['Ene','Feb','Mar','Abr','May','Jun','Jul','Ago','Sep','Oct','Nov','Dic'][fi.getMonth()]} ${fi.getFullYear()} · ${fi.toLocaleTimeString('es-DO',{hour:'2-digit',minute:'2-digit'})}`;
      return `<tr>
        <td><strong>${ev.titulo}</strong>${ev.link ? `<br><a href="${ev.link}" target="_blank" style="font-size:.72rem;color:var(--teal)">Ver link</a>` : ''}</td>
        <td>${TIPO_LABELS_ADMIN[ev.tipo] || ev.tipo}</td>
        <td style="font-size:.78rem">${fecha}</td>
        <td>${ev.es_publico ? '<span class="badge b-active">Público</span>' : '<span class="badge b-pending">Solo inscritos</span>'}</td>
        <td style="font-size:.78rem">${ev.curso_nombre ? (ev.curso_emoji||'')+ ' ' +ev.curso_nombre : '<span style="color:var(--muted)">—</span>'}</td>
        <td style="white-space:nowrap">
          <button class="btn btn-ghost btn-xs" onclick="editarEvento(${ev.id})">Editar</button>
          <button class="btn btn-danger btn-xs" onclick="eliminarEvento(${ev.id})">Eliminar</button>
        </td>
      </tr>`;
    }).join('');

    // Poblar select de cursos en modal
    const sel = document.getElementById('ev-curso');
    if (sel && sel.options.length <= 1) {
      try {
        const rc = await apiFetch('admincurso.php');
        rc.cursos.forEach(c => {
  ...
  } catch(e) { toast('Error al cargar eventos', true); }
}
...
async function abrirModalEvento() {
  ...
  document.getElementById('ev-tipo').value    = 'especial';
  ...
  document.getElementById('ev-publico').checked = true;
  document.getElementById('ev-color').value   = '#0aab96';

  // ✅ Poblar cursos siempre al abrir
  const sel = document.getElementById('ev-curso');
  sel.innerHTML = '<option value="">Público (sin curso)</option>';
  try {
    const rc = await apiFetch('admincurso.php');
    rc.cursos.forEach(c => {
      const opt = document.createElement('option');
...
async function editarEvento(id) {
  try {
    const token = localStorage.getItem('cv_token') || '';
    const r = await fetch(`/Php/eventos.php`, { headers: { Authorization: 'Bearer ' + token } });
    const j = await r.json();
    const ev = j.data.eventos.find(e => e.id === id);
    if (!ev) return toast('Evento no encontrado', true);

    editEventoId = id;
    document.getElementById('modal-evento-title').textContent = 'Editar Evento';
    document.getElementById('ev-titulo').value  = ev.titulo || '';
    document.getElementById('ev-tipo').value    = ev.tipo   || 'especial';
    document.getElementById('ev-curso').value   = ev.curso_id || '';
    document.getElementById('ev-inicio').value  = ev.fecha_inicio ? ev.fecha_inicio.replace(' ','T').slice(0,16) : '';
    document.getElementById('ev-fin').value     = ev.fecha_fin    ? ev.fecha_fin.replace(' ','T').slice(0,16)    : '';
    document.getElementById('ev-link').value    = ev.link || '';
    document.getElementById('ev-desc').value    = ev.descripcion || '';
    document.getElementById('ev-publico').checked = ev.es_publico;
    document.getElementById('ev-color').value   = ev.color || '#0aab96';
    openModal('modal-evento');
  } catch(e) { toast('Error al cargar evento', true); }
}
...
async function guardarEvento() {
  ...
  if (!inicio)  return toast('La fecha de inicio es requerida', true);
  ...
  const body = {
    ...
    fecha_fin:    document.getElementById('ev-fin').value ? document.getElementById('ev-fin').value.replace('T',' ') + ':00' : null,
    link:         document.getElementById('ev-link').value.trim(),
    curso_id:     document.getElementById('ev-curso').value || null,
    es_publico:   document.getElementById('ev-publico').checked,
    color:        document.getElementById('ev-color').value,
  };

  try {
    const method = editEventoId ? 'PUT' : 'POST';
    const ep     = editEventoId ? `eventos.php?id=${editEventoId}` : 'eventos.php';
    const r = await apiFetch(ep, { method, body: JSON.stringify(body) });
    toast('✅ ' + r.mensaje);
...
async function eliminarEvento(id) {
  ...
  try {
    const r = await apiFetch(`eventos.php?id=${id}`, { method: 'DELETE' });
    toast('✅ ' + r.mensaje);
    loadEventos();
  } catch(e) { toast(e.message, true); }
}
...
async function loadCharlas() {
  const grid = document.getElementById('charlas-cursos-grid');
  if (!grid) return;
  try {
    const data = await apiFetch('admincurso.php');
    const cursos = (data.cursos || []).filter(c => c.activo == 1);
    if (!cursos.length) {
      grid.innerHTML = `<div class="card" style="grid-column:1/-1;text-align:center;padding:2.5rem;color:var(--muted)">
        <div style="font-size:1.8rem;margin-bottom:.5rem">📭</div>
    ...
    grid.innerHTML = cursos.map(c => `
      <div class="card" style="display:flex;flex-direction:column;gap:.8rem">
        <div style="display:flex;align-items:center;gap:.8rem">
          <div style="width:46px;height:46px;border-radius:10px;background:rgba(93,230,212,.1);display:flex;align-items:center;justify-content:center;font-size:1.4rem;flex-shrink:0">${c.emoji||'📚'}</div>
          <div style="flex:1;min-width:0">
            <div style="font-family:'Syne',sans-serif;font-weight:800;font-size:.92rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${c.nombre}</div>
            <div style="font-size:.72rem;color:var(--muted);margin-top:.1rem">${c.total_inscritos||0} inscritos · ${c.total_workshops||0} workshops</div>
          </div>
        </div>
        <div class="prog-bar"><div class="prog-fill" style="width:${c.progreso_promedio||0}%"></div></div>
        <div style="display:flex;gap:.5rem;margin-top:.2rem">
          <a href="/Charla-admin.php?curso=${encodeURIComponent(c.slug)}"
             style="flex:1;display:flex;align-items:center;justify-content:center;gap:.4rem;background:rgba(239,68,68,.1);border:1px solid rgba(239,68,68,.25);color:#f07a7a;font-family:'Syne',sans-serif;font-weight:700;font-size:.75rem;padding:.5rem .8rem;border-radius:100px;text-decoration:none;transition:background .15s"
    ...
    grid.innerHTML = `<div class="card" style="grid-column:1/-1;text-align:center;padding:2rem;color:var(--muted)">Error al cargar cursos</div>`;
  }
}
...
(async () => {
  try {
    ...
    // Inicializar toggles del live
    syncToggle('live-activo', 'live-activo-track', 'live-activo-thumb');
    syncToggle('live-chat',   'live-chat-track',   'live-chat-thumb');

    // Nombre del admin en topbar
    try {
      const pr = await fetch('/perfil.php', { credentials: 'include' });
